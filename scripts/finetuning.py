...
def main(args, m_config, o_config):
    ...
    enable_mlflow = args.enable_mlflow and not no_mlflow
    ...
    if enable_mlflow:
        ...
        # Resolve parent run: prefer env var (set by sweep orchestrator/Docker),
        # then fall back to finding or creating a run named after group_id.
        parent_run_id = os.environ.get("MLFLOW_PARENT_RUN_ID")
        if parent_run_id is None and args.group_id:
            exp = mlflow.get_experiment_by_name(experiment)
            if exp is not None:
                runs = mlflow.search_runs(
                    experiment_ids=[exp.experiment_id],
                    filter_string=f"tags.mlflow.runName = '{args.group_id}' AND attributes.status = 'RUNNING'",
                    output_format="list",
                )
                if runs:
                    parent_run_id = runs[0].info.run_id
            if parent_run_id is None:
                parent_run = mlflow.start_run(run_name=args.group_id, tags={"group_id": args.group_id})
...
def build_argparser():
    ...
    parser.add_argument("--log-checkpoints", "--log_checkpoints", action="store_true", help="Log IVON checkpoints to MLflow artifacts")
    parser.add_argument("--group-id", type=str, default="finetuning_test", help="Tag runs with a shared MLflow group id")
    parser.add_argument("--uid", type=str, default=None, help="Optional MLflow run name (auto-generated if omitted)")
    return parser
