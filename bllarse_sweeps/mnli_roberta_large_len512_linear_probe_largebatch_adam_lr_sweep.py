...
LEARNING_RATES = [1e-4, 3e-4, 6e-4, 1e-3]
...
BASE: Dict[str, Any] = dict(
    ...
    reuse_cache=True,
    enable_mlflow=True,
    group_id="mnli_roberta_large_len512_linear_probe_largebatch_adam_lr_sweep",
        ...
        for train_batch_size in BATCH_SIZES:
            for seed in SEEDS:
                uid = (
                    ...
                    f"adam_lr{learning_rate:.1e}_bs{train_batch_size}_seed{seed}_ep140"
                )
                configs.append(
                    dict(
                        **BASE,
