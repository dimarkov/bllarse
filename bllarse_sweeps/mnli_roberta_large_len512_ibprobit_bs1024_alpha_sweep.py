...
BASE: Dict[str, Any] = dict(
    ...
    reuse_cache=True,
    enable_mlflow=True,
    group_id="mnli_roberta_large_len512_ibprobit_bs1024_alpha_sweep",
...
def create_configs() -> List[Dict[str, Any]]:
    ...
    for alpha in ALPHAS:
        alpha_tag = f"{alpha:.0e}".replace("-", "m")
        for num_update_iters in NUM_UPDATE_ITERS:
            for seed in SEEDS:
                uid = (
                    "mnli_roberta_large_len512_"
                    f"cavi_bs1024_alpha{alpha_tag}_iters{num_update_iters}_seed{seed}_ep1"
                )
                configs.append(
                    dict(
                        **BASE,
                        uid=uid,
                        ibprobit_alpha=alpha,
