...
def _resolve_run_dir(run_dir: str) -> Path:
    """Accept absolute/relative run_dir or just the legacy W&B run name."""
    path = Path(run_dir)
    if path.exists():
        return path.resolve()
...
def restore_full_network_checkpoint(
    *,
...
def main():
    ...
    logits, _, ckpt_path = restore_full_network_checkpoint(
        run_id=args.run_id,
...
def _list_mlflow_checkpoints(client: MlflowClient, run_id: str, artifact_root: str) -> list[str]:
    ...
    return [info.path for info in infos if info.path.endswith(".eqx")]
